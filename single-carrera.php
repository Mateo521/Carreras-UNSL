            ...
            <div class="max-w-3xl">
                ...
                <div class="flex flex-wrap items-center gap-4 mb-4">
                    ...
                    if ($terms_facultad && !is_wp_error($terms_facultad)) {
                        foreach ($terms_facultad as $term_fac) {
                            $f_slug = strtolower($term_fac->slug);
                            $f_nombre = isset($nombres_facultades[$f_slug]) ? $nombres_facultades[$f_slug] : $term_fac->name;
                            $f_bg = isset($colores_facultades[$f_slug]) ? $colores_facultades[$f_slug]['bg'] : 'bg-white';
                ...
                <div class="flex flex-wrap gap-3 mt-8">
                    <div class="flex items-center gap-2 bg-[#ffffff0d] border border-[#ffffff12] px-4 py-2.5 rounded-lg backdrop-blur-sm">
                        <svg class="w-4 h-4 text-[#88CAFC] shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
                        <span class="text-white text-sm font-medium"><?php echo esc_html($duracion); ?></span>
                    </div>
                    <div class="flex items-center gap-2 bg-[#ffffff0d] border border-[#ffffff12] px-4 py-2.5 rounded-lg backdrop-blur-sm">
                        <svg class="w-4 h-4 text-[#88CAFC] shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0z" /><path d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0z" /></svg>
